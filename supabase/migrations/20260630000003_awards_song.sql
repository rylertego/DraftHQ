-- Awards ceremony music: optional commissioner-chosen song (WalkUpSong JSONB,
-- same shape as team walk-up songs — YouTube/Spotify via the song picker).
-- Null = the built-in default ceremony track.

alter table public.drafts
  add column if not exists awards_song jsonb;

-- Drop and recreate update_draft_extras with the new params.
-- p_clear_awards_song exists because coalesce() cannot set a column back to
-- null; all params default to null so existing callers need no changes.
drop function if exists public.update_draft_extras(uuid, text, boolean, integer, boolean, timestamptz, text, jsonb, integer, text, integer, text, text, text[], text[], boolean, text, text, boolean, integer, boolean, text, text);
...
revoke all on function public.update_draft_extras(uuid, text, boolean, integer, boolean, timestamptz, text, jsonb, integer, text, integer, text, text, text[], text[], boolean, text, text, boolean, integer, boolean, text, text, jsonb, boolean) from public, anon;
grant execute on function public.update_draft_extras(uuid, text, boolean, integer, boolean, timestamptz, text, jsonb, integer, text, integer, text, text, text[], text[], boolean, text, text, boolean, integer, boolean, text, text, jsonb, boolean) to authenticated;
