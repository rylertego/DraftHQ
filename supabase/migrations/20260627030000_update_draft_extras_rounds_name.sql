-- Extend update_draft_extras to cover rounds, name, and team_count so client
-- code never needs a direct UPDATE on public.drafts (which has no client grant).
...
revoke all on function public.update_draft_extras(uuid, text, boolean, integer, boolean, timestamptz, text, jsonb, integer, text, integer) from public, anon;
grant execute on function public.update_draft_extras(uuid, text, boolean, integer, boolean, timestamptz, text, jsonb, integer, text, integer) to authenticated;
