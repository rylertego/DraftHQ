-- Commissioner setting: how each team's walk-up music behaves across turns.
-- 'restart' (default) = start fresh each turn (current behavior).
-- 'resume'  = each team's song continues from where it left off on its
--             previous turn; position is derived client-side from pick
--             timestamps, so no playback state is stored here.

alter table public.drafts
  add column if not exists walk_up_music_mode text not null default 'restart'
    check (walk_up_music_mode in ('restart', 'resume'));

-- Drop and recreate update_draft_extras with the new param.
-- All params default to null so existing callers need no changes.
drop function if exists public.update_draft_extras(uuid, text, boolean, integer, boolean, timestamptz, text, jsonb, integer, text, integer, text, text, text[], text[], boolean, text, text, boolean, integer, boolean, text);
...
revoke all on function public.update_draft_extras(uuid, text, boolean, integer, boolean, timestamptz, text, jsonb, integer, text, integer, text, text, text[], text[], boolean, text, text, boolean, integer, boolean, text, text) from public, anon;
grant execute on function public.update_draft_extras(uuid, text, boolean, integer, boolean, timestamptz, text, jsonb, integer, text, integer, text, text, text[], text[], boolean, text, text, boolean, integer, boolean, text, text) to authenticated;
