-- Add landmine detection to record_draft_pick (the actual pick execution path).
-- The 3-param make_pick overload delegates here, so this is where is_landmine must be stamped.
create or replace function public.record_draft_pick(p_draft_id uuid, p_player_id uuid, p_commissioner_override boolean)
...
begin
  ...
  select * into v_team from public.teams where draft_id = p_draft_id and draft_position = v_draft_position;
  ...
  if p_commissioner_override then
    ...
    select * into v_participant from public.draft_participants
    where draft_id = p_draft_id and user_id = v_user_id and team_id = v_team.id and role in ('commissioner', 'owner');
  end if;

  if not found then
    raise exception using errcode = '42501', message = case
      when p_commissioner_override then 'Commissioner participant not found.'
  ...
  set
    ...
    pick_deadline_at = case when v_draft.current_pick = v_total_picks then null else now() + make_interval(secs => pick_seconds) end,
